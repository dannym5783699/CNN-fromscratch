...
def get_data_loader(is_train, subset_size=None):
    ...
    dataset = CIFAR10(root="cifar10_data", train=is_train, transform=transform, download=True)
...
train_loader = get_data_loader(True, 128)
test_loader = get_data_loader(False, 128)
...
for i in range(6):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title(f'Train vs Test Accuracy Over Epochs Batch Size={batch_size} CIFAR')
    plt.legend()
    plt.grid(True)
    plt.show()
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title('Combined Accuracy Over Epochs CIFAR')
plt.legend()
plt.grid(True)
plt.show()
