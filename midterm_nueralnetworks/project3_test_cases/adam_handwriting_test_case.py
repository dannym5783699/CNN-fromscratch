...
def batch_size_test_with_adam():
    """
    Train the network with Adam optimizer using different batch sizes.
    """
    ...
    for batch_size, ax in zip(batch_sizes, axs.flatten()):
        ...
        ax.set(
            xlabel="Epoch",
            ylabel="Log Loss",
            title=f"Batch Size: {batch_size} with Adam"
        )
        ax.set_yscale('log')
        ax.legend()
