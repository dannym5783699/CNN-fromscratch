...
def batch_size_test_with_newtons():
    """
    Train the network with Newton's Method using different batch sizes,
    ...
    MAX_EPOCHS = 100
    LR = 1e-2  # Learning rate (for scaling Newton's updates)
    LAMBDA_REG = 1e-5  # Regularization for Hessian
    ...
    for batch_size, ax in zip(batch_sizes, axs.flatten()):
        ...
        print(f"Training model with batch size {batch_size} using Newton's Method")

        # Initialize progress bar for epochs
        with tqdm(total=MAX_EPOCHS, desc=f"Batch Size: {batch_size}") as pbar:
            for epoch in range(MAX_EPOCHS):
                ...
                for x_batch, y_batch in get_batches(X_train, Y_train, batch_size):
                    ...
                    net.newtons_method(LR, lambda_reg=LAMBDA_REG)  # Use Newton's Method
                    net.zero_grad()
                ...
                pbar.set_postfix({"Train Loss": mean_train_loss, "Test Loss": test_loss})
                pbar.update(1)
        ...
        ax.set(
            xlabel="Epoch",
            ylabel="Log Loss",
            title=f"Batch Size: {batch_size} with Newton's Method"
        )
        ax.set_yscale('log')
        ax.legend()
