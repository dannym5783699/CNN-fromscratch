...
def batch_size_test_he_weight():
    ...
    for batch_size, ax in zip(batch_sizes, axs.flatten()):
        ...
        net = FeedforwardNeuralNetwork([
            Layer(2, 10, "relu"),  # Input layer: 2 features, 10 neurons
            Layer(10, 10, "relu"),  # Hidden layer: 10 neurons
            Layer(10, 2, "softmax", final_layer=True)  # Output layer: 2 classes, softmax
        ])
...
def batch_size_test_xavier_uniform():
    ...
    for batch_size, ax in zip(batch_sizes, axs.flatten()):
        # Initialize the network
        net = FeedforwardNeuralNetwork([
            Layer(2, 10, "relu", weight_init="xavier_uniform"),  # Input layer: 2 features, 10 neurons
            Layer(10, 10, "relu", weight_init="xavier_uniform"),  # Hidden layer: 10 neurons
